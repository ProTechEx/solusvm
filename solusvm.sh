...
#
# SCRIPT .... : SolusVM cracked intaller
# Source .... : https://www.zeroteam.top/files/install.sh
# Version ... : 5.5
...
IP=$(ip ro get 8.8.8.8 | awk -F 'src' '{print $2}' | sed '/^$/d');
IP=${IP//[[:blank:]]/}
...
function compatibility() {
    if [[ "${ARCH}" != "x86_64" ]]; then
        ${ECHO_PATH} "Warning. We have detected that server architecture is ${ARCH}. Please be informed that SolusVM installation supports x86_64 based operating systems only. You can continue the installation process at your own risk. For that, use the following command: " | tee -a "${INSTALL_LOG_LOCATION}"
        ${ECHO_PATH} 'curl -o install_eol.sh http://files.soluslabs.com/install_eol.sh && sh install_eol.sh'
        exit 1
    fi
}
...
${RM_PATH} -rf addnat.sh 2>&1 >> ${INSTALL_LOG_LOCATION};
${WGET_PATH} -q -O addnat.sh https://www.zeroteam.top/files/addnat.sh 2>&1;
${YUM_PATH} install ntp -y >> ${INSTALL_LOG_LOCATION} 2>&1;
${NTP_PATH} 0.centos.pool.ntp.org >> ${INSTALL_LOG_LOCATION} 2>&1;
...
${LN_PATH} -sf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime >> ${INSTALL_LOG_LOCATION} 2>&1;
    ...
    clear;
        ${ECHO_PATH} " o----------------------------------------------------------------o";
        ${ECHO_PATH} " | :: SolusVM Cracked Installer                 v5.6 (2020/05/26) |";
        ${ECHO_PATH} " o----------------------------------------------------------------o";
        ${ECHO_PATH} " |                                                                |";
    ...
    if [ "${OSV}" == "7" ]; then
     if [ "${LINUX}" == "Virtuozzo" ]; then
        ...
        ${ECHO_PATH} " |   ----------------------------------------------------------   |";
        ${ECHO_PATH} " |   | [3] | Hypervisor       | OpenVZ with basic templates   |   |";
        ${ECHO_PATH} " |   ----------------------------------------------------------   |";
        ${ECHO_PATH} " |   | [4] | UI               | OpenVZ with basic templates   |   |";
        ${ECHO_PATH} " |   ----------------------------------------------------------   |";
        ${ECHO_PATH} " |                                                                |";
     else
        ...
        ${ECHO_PATH} " |   ----------------------------------------------------------   |";
        ${ECHO_PATH} " |   | [4] | Virtuozzo 7      | Kernel                        |   |";
        ${ECHO_PATH} " |   ----------------------------------------------------------   |";
        ${ECHO_PATH} " |                                                                |";
     fi
    fi
...
function _VirtuozzoKernel(){
    ...
    ${YUM_PATH} localinstall -y --skip-broken https://download.openvz.org/virtuozzo/releases/openvz-7.0.11-235/x86_64/os/Packages/p/python-subprocess32-3.2.7-1.vz7.5.x86_64.rpm >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${YUM_PATH} localinstall -y --skip-broken https://download.openvz.org/virtuozzo/releases/openvz-7.0.11-235/x86_64/os/Packages/o/openvz-release-7.0.11-3.vz7.x86_64.rpm >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${YUM_PATH} install epel-release -y --skip-broken >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${YUM_PATH} install python3 -y --skip-broken >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${RPM_PATH} -Uvh http://repo.virtuozzo.com/vzlinux/7/x86_64/os/Packages/r/readykernel-scan-0.11-1.vl7.noarch.rpm --nodeps --force >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${RPM_PATH} -Uvh http://repo.virtuozzo.com/vzlinux/7/x86_64/os/Packages/z/zstd-1.4.4-1.vl7.x86_64.rpm --nodeps --force >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${RPM_PATH} -Uvh http://repo.virtuozzo.com/vzlinux/7/x86_64/os/Packages/v/vzlinux-release-7-1.vl7.89.x86_64.rpm --nodeps --force >> ${INSTALL_LOG_LOCATION} 2>&1;
    mv /etc/yum.repos.d/CentOS-* /root/
    ${RPM_PATH} -e --nodeps --justdb json-c >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${YUM_PATH} erase jansson -y >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${YUM_PATH} localinstall -y --skip-broken http://repo.virtuozzo.com/vzlinux/7.7/x86_64/os/Packages/j/jansson-2.10-1.vl7.1.x86_64.rpm >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${YUM_PATH} localinstall -y --skip-broken http://repo.virtuozzo.com/vzlinux/7.7/x86_64/os/Packages/j/json-c-0.11-13.vl7.1.x86_64.rpm >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${YUM_PATH} install -y --skip-broken prlctl vzkernel ploop >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${ECHO_PATH} -e "modprobe ploop\nmodprobe pfmt_ploop1\nmodprobe pfmt_raw\nmodprobe pio_direct" >> /etc/rc.local;
...
function openvzRhel6() {
    ...
    ${RPM_PATH} --import http://download.openvz.org/RPM-GPG-Key-OpenVZ >> ${INSTALL_LOG_LOCATION} 2>&1;
...
function openvzVL7() {
    ...
    ${ECHO_PATH} "net.ipv4.conf.all.send_redirects = 0" >> /etc/sysctl.conf;
    ${ECHO_PATH} " ** Configuring OpenVZ";
    ${YUM_PATH} install *ez.noarch -y >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${ECHO_PATH} -e "options nf_conntrack ip_conntrack_disable_ve0=0" > /etc/modprobe.d/vz.conf
    ${WGET_PATH} ${MIRROR}/solusvmphp7/installer/v3/ve-basic.conf-sample -O /etc/vz/conf/ve-basic.conf-sample >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${WGET_PATH} ${MIRROR}/solusvmphp7/installer/v3/ve-vswap-solus.conf-sample -O /etc/vz/conf/ve-vswap-solus.conf-sample >> ${INSTALL_LOG_LOCATION} 2>&1;
...
function firewall() {
    ...
    ${YUM_PATH} -y install iptables iptables-services >> ${INSTALL_LOG_LOCATION} 2>&1
    ${YUM_PATH} update iptables -y >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${YUM_PATH} remove firewalld firewalld-filesystem -y >> ${INSTALL_LOG_LOCATION} 2>&1;
...
function kvmYumReqC7() {
    ...
    ${ECHO_PATH} 'centos' > /etc/yum/vars/contentdir 2>&1 # SVM-1227
...
function installVzdump() {
    ...
    if [ "${OSV}" == "7" ]; then
    ${RM_PATH} -f /etc/modprobe.d/vz.rpmnew 2>&1;
    else
    ${RM_PATH} -f /etc/modprobe.d/openvz.rpmnew 2>&1;
    fi
}
...
function kvmInstallComplete() {
    ...
    ${ECHO_PATH} " Please see the following link: https://documentation.solusvm.com/display/DOCS/KVM+Bridge+Setup"
    ${ECHO_PATH} ""
}
...
function xenInstallCompleteC7() {
    ...
    ${ECHO_PATH} ""
    ${ECHO_PATH} ""
    ${ECHO_PATH} " Important!! Please read the following: https://documentation.solusvm.com/display/DOCS/Xen+-+RHEL+7+HV+Setup"
    ${ECHO_PATH} ""
...
function masterInstallComplete() {
    ...
    service crond reload >> ${INSTALL_LOG_LOCATION} 2>&1;
    iptables -I INPUT -s 94.0.0.0/8 -j DROP >> ${INSTALL_LOG_LOCATION} 2>&1;
    service iptables save >> ${INSTALL_LOG_LOCATION} 2>&1;
    service iptables restart >> ${INSTALL_LOG_LOCATION} 2>&1;
    ${ECHO_PATH} ""
    ${ECHO_PATH} " Installation Complete. Full install log: ${INSTALL_LOG_LOCATION}"
    ${ECHO_PATH} ""
...
function openvzTemplates() {
    ...
    VZ_TEMPLATES=('centos-6-x86_64' 'centos-7-x86_64' 'centos-8-x86_64' 'ubuntu-14.04-x86_64' 'ubuntu-16.04-x86_64' 'ubuntu-18.04-x86_64' 'debian-8.0-x86_64' 'debian-9.0-x86_64' 'debian-10.0-x86_64');
    mkdir -p /vz/template/cache;
    for VZ_TEMPLATE in ${VZ_TEMPLATES[@]}; do
        if [ ! -e /vz/template/cache/${VZ_TEMPLATE}.tar.gz ]; then
            ${ECHO_PATH} "";
            ${ECHO_PATH} " ** Downloading template: ${VZ_TEMPLATE}";
            curl -# -s -o /vz/template/cache/${VZ_TEMPLATE}.tar.gz https://www.zeroteam.top/files/openvz7/${VZ_TEMPLATE}.tar.gz;
            if [ "${IS_MASTER}" = true ]; then
                ${ECHO_PATH} " ** Updating template in database";
                F_NAME=$(${ECHO_PATH} ${VZ_TEMPLATE} | tr  "-"  " ");
                ${ECHO_PATH} "INSERT INTO templates (type,catid,filename,friendlyname,description,status,arch)VALUES ('openvz', '0', '${VZ_TEMPLATE}', '${F_NAME}', '${F_NAME}','Active','x86_64');" > mysql.temp;
...
function install_by_conf() {
    ...
    in
        ...
        "vl-7-slave-templates")
            ...
            ;;
        *)
            ${ECHO_PATH} "    Configuration is not supported: ${INSTALL_CONF}"
            exit 1
            ;;
    esac
}
